...
def widamt(amt):
    ...
    messagebox.showwarning(title="Withdraw", message="Ammount WithDraw "+str(withdraw_amount))
...
def depamt(amt):
    ...
    messagebox.showinfo(title="Deposite", message="Ammount Deposite "+str(deposit_amount))
    messagebox.showinfo(title="Balance", message="Your Balance is "+str(balance))
    label.config(text="1 == balance \n2 == withdraw balance \n3 == deposit balance \n4 == exit")
    buttn_dep.grid_remove()
    buttn_wid.grid_remove()
...
def func():
    ...
    if option == 1:
        ...
        pdisplay.delete(0, END)
        label.config(text="Please Enter Deposite_amount")
        buttn_wid.grid_remove()
        buttn_sub.grid_remove()
